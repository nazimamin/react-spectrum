...
A `CalendarDateTime` object represents an exact date and time in a specific time zone, in a specific calendar system such as the Gregorian calendar. Use this type to represent an exact moment in time at a particular location on Earth.
...
You can also create a `CalendarDate` by parsing an [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) formatted string using the one of the following functions:
...
See the [Calendar](Calendar.html) docs for details about the supported calendars.
...
A list of valid era identifiers can be retried using the `getEras` method of a `Calendar` instance. If an era is not provided, the date is assumed to be in the current era.
...
date.add({weeks: 1}); // 2022-02-09T09:45[America/Los_Angeles]
...
let date = new CalendarDate(2022, 2, 3, 23);
